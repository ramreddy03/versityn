...
//, (select GROUP_CONCAT(pc.comment) AS comment from post_comments as pc where pc.post_id = std.id ORDER BY pc.created_at ) as comment, (select GROUP_CONCAT(sd.username) from students as sd where sd.id IN (select pc.user_id from post_comments as pc where pc.post_id = std.id ORDER BY pc.created_at)) as studentname
   ...
   // echo "<pre>"; var_dump($studentspostdocs); exit();
	...
	{		
		...
		if(!empty($_POST['post']) && empty($_FILES['files']['name'][0])) {
			$data = array(
		...
		$insert_qry = $this->db->insert('studentspost_docs', $data);
		//redirect(current_url());
		 echo (!empty($insert_qry)) ? "success" : "failed";
		}
   ...
   public function countryserach(){
   		$post_id = $this->input->post('country_id', TRUE);
		if(!empty($post_id)) {
			if($post_id == 'all') {
		     	 $queryresult = $this->commonclass->execute_rawquery("SELECT * FROM university");
 				$masteruniversity = $queryresult->result_array();
		 	 } else {
		 	 	$keyword = !empty($post_id) ? $post_id : '';
		...
		$i = 0;
			...
			$university .='<option value="">Select university</option><option value="all">Select All</option>';
   ...
   public function connections(){
    ...
    if(!empty($_POST)){
      if($_POST['submit'] == 'Go'){
      	if( $_POST['university'] == 'all'){
      		if($_POST['country'] == 'all') {      			
      			$where = array('sd.id !=' => $this->sessiondata['userid']);      	
	    	} else if($_POST['country'] != 'all' && !empty($_POST['country']) )
	    	 {
	    	$where = array('sd.country =' => $_POST['country'], 'sd.id !=' => $this->sessiondata['userid']);
	    	}	    	
      	} else {
	        $Search_university  = !empty($_POST['university']) ? $_POST['university']: '';	        
	        $where = array('sd.id_university =' => $Search_university, 'sd.id !=' => $this->sessiondata['userid']);	        
   		}
   		$Search_university_res = $this->db->select('sd.*,uni.universityname')->from('students as sd');
		$Search_university_res = $this->db->join('university as uni', 'uni.id = sd.id_university','left');
		$Search_university_res = $this->db->where($where);
		$Search_university_res = $this->db->get()->result_array();
   ...
   // echo"<pre>"; print_R($Search_university_results); echo"</pre>";
    ...
    $data['loginid'] = $this->sessiondata['userid'];
    $data['Searchuniversityresults'] = $Search_university_res;
    $data['friends_requests'] = $friends_requests;
