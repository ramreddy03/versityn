                                ...
                                <table class="table table-bordered sortableTable responsive-table" id ="dashboard-table">
                                    <thead>
                                        <tr>
                                            ...
                                            <th width="17%">E-mail<i class="icon-sort"></i><i class="icon-sort-down"></i> <i class="icon-sort-up"></i></th>
                                            <th width="17%">Escalation e-mail<i class="icon-sort"></i><i class="icon-sort-down"></i> <i class="icon-sort-up"></i></th>
                                            <th width="10%">Role<i class="icon-sort"></i><i class="icon-sort-down"></i> <i class="icon-sort-up"></i></th>
                                            <th width="15%">Actions<i class="icon-sort"></i><i class="icon-sort-down"></i> <i class="icon-sort-up"></i></th>
                                        </tr>
                                    </thead>
                                    <tbody>
										...
										<?php
											if (count($userslist)>0) {
												foreach ($userslist as $onerec) {
													...
													$viewdetailslink = anchor("admin/cpanel/usersform/".$onerec['id_user'], $onerec['firstname'], array('title'=>'Edit details'));
                                    ...
                                    <tbody>
										...
										<?php
											if (count($tempuserslist)>0) {
												foreach ($tempuserslist as $onerec) {
													...
													$createlink = '<a style="cursor:pointer" id="'.$onerec['id_user'].'" onclick="createuser('.$onerec['id_user'].', \''.$onerec['username'].'\');">Create</a> ';
										...
										?>
										 <tr>
                                            ...
                                            <td class=" <?php echo $tdclass; ?>"><?php echo $onerec['escalation_email']; ?></td>
                                            <td class=" <?php echo $tdclass; ?>"><?php echo !empty($roleslist[$onerec['id_role']]) ? $roleslist[$onerec['id_role']] : ""; ?></td>
                                            <td class=" <?php echo $tdclass; ?>"><?php echo $createlink; ?></td>
                                        </tr>
...
<script type="text/javascript">
	...
	function createuser(id_user, dispname) {
		if (confirm("Are you sure to create and notify account details for "+dispname+"?")) {
